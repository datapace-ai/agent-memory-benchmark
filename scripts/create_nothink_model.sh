...
cd "$(dirname "$0")/.."
mkdir -p models
ollama show qwen3:14b --modelfile > models/Modelfile.qwen3-14b.orig
python3 - <<'PY'
import re
src = open("models/Modelfile.qwen3-14b.orig").read()
...
assert src != before, "template anchors not found; inspect models/Modelfile.qwen3-14b.orig"
...
open("models/Modelfile.qwen3-nothink", "w").write(src)
print("Modelfile written")
PY
ollama create qwen3-nothink:14b -f models/Modelfile.qwen3-nothink >/dev/null 2>&1
